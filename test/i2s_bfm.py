...
async def monitor_i2s_bus(clk, bclk_sig, ws_sig, sdata_sig, callback):
    """Monitors serial I2S output bus using system clock edge detection to avoid VPI part-select callback issues."""
    shift_reg = 0
    bit_cnt = 0
    ws_latched = 1
    prev_bclk = 0
    ...
    while True:
        # Sincronizzazione sul clock di sistema (evita callback VPI sulle fette di bus)
        await RisingEdge(clk)

        try:
            curr_bclk = int(bclk_sig.value)
            curr_ws = int(ws_sig.value)
            curr_sdata = int(sdata_sig.value)
        except ValueError:
            continue
        ...
        if curr_bclk == 1 and prev_bclk == 0:
            if curr_ws != ws_latched:
                ...
                if bit_cnt == 16:
                    signed_val = shift_reg if shift_reg < 0x8000 else shift_reg - 0x10000
